...
# Raw data set (done by Stata)  
# Data preparation (done by Stata)  
# Read-in data set.  
# Optimization  
# 
# Calibration process:    
# CRRA utility function vs Loss-aversion function: Final step CRRA explains the far and LA explains the middle.     
# Probability-weighting function vs Sigma-mu transformation: Use PW for empirical and use SM for theoretical.    
# Asymmetric probability-weighting function and symmetric function.    
# Piece-wise linear contract vs optimal contract.

# # 2. Definitions

# ## 2.1. Common functions for pincipal and agent

# ### Global variables
# $\eta_s$ and $\eta_m$ set to zero means no probability weighting.
...
eta_s = 0
eta_m = 0
# eta_s = 1.7516001425989953
# eta_m = 0.32
wealth_min = 0.001


# ### Objective production function $P_T$

# \begin{align*}
# P_{T}= \exp\left\{\ln{P_0} + \left(r-d\right)T - \frac{s^{2}}{2}T +us\sqrt{T}\right\}
# \end{align*}
...
# ### Piece-wise contract shape

# \begin{align*}
# W_{T}=(\phi+\omega_{0})e^{rT}+n_{s}e^{dT}P_{T}+n_{o}\max\left\{ P_{T}-K,0\right\} 
# \end{align*}

# In[ ]:

def f_W_T_pc(v, P_0, r_f, d, s, T, wealth, phi, n_s, n_o, K):
    """
    The wealth to the agent given the stock price at time T.
    pc: piecewise-linear contract.
    """
...
# \begin{align*}
# \frac{dW_T}{dP_T} = n_{s}e^{dT} + n_{o}\mathbb{I}_{P_{T}>K}
# \end{align*}
